...
contract PIController is SafeMath, SignedSafeMath {
    ...
    // This value is multiplied with errorDerivative
    int256 public kd;                                      // [EIGHTEEN_DECIMAL_NUMBER]
    ...
    */
    function getBoundedPidOutput(int pidOutput) public  view returns (int256) {
        if (pidOutput < outputLowerBound) {
            return outputLowerBound;
        } else if (pidOutput > outputUpperBound) {
    ...
    * @param boundedPidOutput The bounded output computed from the error, integral and derivative terms
    ...
    */
    function clampErrorIntegral(int boundedPidOutput, int newErrorIntegral, int newArea) internal view returns (int256) {
        if (both(both(boundedPidOutput == outputLowerBound, newArea < 0), errorIntegral < 0)) {
          return subtract(newErrorIntegral, newArea);
        } else if (both(both(boundedPidOutput == outputUpperBound, newArea > 0), errorIntegral > 0)) {
          return subtract(newErrorIntegral, newArea);
        }
    ...
    */
    ...
    /*
    * @notice Compute error derivative
    * @param error The system error
    */
    function getErrorDerivative(int error) public  view returns (int256) {
        uint256 elapsed = (lastUpdateTime == 0) ? 0 : subtract(now, lastUpdateTime);
        if (elapsed == 0) {
            return 0;
        }

        return divide(subtract(error, lastError), elapsed);
    }
    ...
    */
    function getRawPidOutput(int error, int errorIntegral, int errorD) public  view returns (int256, int256, int256) {
        // output = P + I + D= Kp * error + Ki * errorIntegral + Kd * errorD
        ...
        int dOutput = multiply(errorD, int(kd)) / int(EIGHTEEN_DECIMAL_NUMBER);
        return (addition(coBias, addition(addition(pOutput, iOutput)), dOutput), pOutput, iOutput, dOutput);
    }
    ...
    */
    function update(int error) external returns (int256, int256, int256) {
        ...
        int256 errorD = getErrorDerivative(error);

        (int256 pidOutput, int256 pOutput, int256 iOutput, int256 dOutput) = getRawPidOutput(error, newErrorIntegral, errorD);
        
        int256 boundedPidOutput = getBoundedPidOutput(pidOutput);

        // If output has reached a bound, undo integral accumulation
        errorIntegral = clampErrorIntegral(boundedPidOutput, newErrorIntegral, newArea);

        lastUpdateTime = now;
        lastError = error;
        numObservations += 1;

        return (boundedPidOutput, pOutput, iOutput, dOutput);
    ...
    * @param error The system error
    */
    function getNextPidOutput(int error) public view returns (int256, int256, int256) {
        (int newErrorIntegral, int newArea) = getNextErrorIntegral(error);
        int errorD = getErrorDerivative(error);
        (int pidOutput, int pOutput, int iOutput, int dOutput) = getRawPidOutput(error, newErrorIntegral, errorD);
        int boundedPidOutput = getBoundedPidOutput(pidOutput);
